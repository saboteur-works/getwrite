...
A PreToolUse Bash hook, `.claude/hooks/guard-worktree-git.py` (registered in
`.claude/settings.json`), separately blocks whole-tree `git checkout`/`restore … .`
and `git reset --hard <other ref>` in the main worktree, and — while
`.claude/.pipeline-active` exists — also blocks bare `git reset --hard`, `git switch`,
and `git checkout <existing ref>`; single-path restores, `git show <ref>:<path>`,
`git worktree add`, and other worktrees are unaffected. It exists because a subagent
once overwrote the main worktree from another ref during a pipeline run.
...
- **Storage**: `projects/` at the repo root contains one UUID-named folder per project. Each project directory contains:
  ...
  - `meta/relationships.json` — authored, typed, directed edges between declared entities (see [Glossary: Entity relationship](#glossary))
...
- **API routes** (`frontend/app/api/`): Read/write the filesystem directly. Top-level groups: `projects`, `project/*` (id, delete, rename, tags, preferences, editor-config, metadata-schema, revision-settings, query, features, entity-mention-counts, entity-cooccurrence, entity-relationships), `project-resources`, `project-types`, `resource/*` (id, revision, upload, mentions, mentioned-in), `compile`, `export`, `version-check` (Electron update check), `encryption` (lock state + unlock/lock/enable/export/resume; the keyring is
...
frontend/
  ...
  components/            # Feature-organized React components
    ...
    WorkArea/            # EditView / DataView / DiffView + Views/{Organizer,Timeline,EntityRoster,EntityRelationshipGraph}
...
- _Entities & mentions_: `entity-detection.ts` (word-boundary/possessive/plural alias matching, offline and pure), `entity-alias-table.ts` (per-project table of every entity's name + aliases, plus cross-entity ambiguity detection), `entity-alias-warnings.ts` (non-blocking short/common-word alias noise heuristics), `mention-index.ts` (mention index schema and persistence under `meta/index/mentions.json`, rebuilt from scratch by `indexer-queue.ts` and the `reindex` CLI), `mentions-core.ts` (transport-agnostic reads over the mention index, merging in explicit `linkedFrom` backlinks and ambiguity flags for the entity's own "mentioned in" view, plus `getProjectMentionCounts` — per entity, its prose-occurrence total and the number of distinct resources those occurrences span, for the entity roster — and `getEntityCooccurrence` — per declared entity, every other declared entity it shares at least one resource with by detected mention, with the shared-resource count and ids, for the "Also appears with" list on the entity's own sidebar view), `entity-relationships.ts` (persistence for authored, typed, directed relationship edges between two declared entities under `meta/relationships.json` — `loadEntityRelationships`, `createEntityRelationship` (idempotent on the `(sourceEntityId, targetEntityId, relationshipType)` triple, rejects a same-entity edge via `SameEntityRelationshipError` and a `relationshipType` outside the project's configured list via `InvalidRelationshipTypeError`), `removeEntityRelationship`, and `removeEntityRelationshipsForEntity` (bulk-deletes every edge naming a given entity as source or target, in one `withMetaLock` call, returning the removed count); the read-modify-write sequence for all three mutators runs inside a single `withMetaLock` call)
...
- _Scrivener import (Feature 31)_: `scrivener/` — the `project import-scrivener` pipeline, also driven from the desktop UI (Feature 43; see `electron/src/scrivener-import/` and the Electron Shell section below). `scrivx-types.ts` (typed shape of a parsed `.scrivx`), `scrivx-parser.ts` (parses `.scrivx`'s real Scrivener 3 element shapes — child `<Title>` elements, `<FieldID>`/`<Value>` metadata pairs, `<CustomMetaDataSettings>`, `<Keywords><KeywordID>`, Label/Status names as element text — via `fast-xml-parser`, a direct dependency added for this parser; `isSupportedScrivenerProject` implements the FR-2 Creator allow-list check — only a `Creator` starting with `SCRMAC-3` is accepted; a malformed fragment is recorded as a recoverable `fragmentErrors` skip rather than aborting the parse), `rtf-to-tiptap.ts` (hand-rolled RTF → TipTap converter scoped to the control words the spec measured — bold/italic marks, `\line` as a `hardBreak` node, backslash-newline as an added paragraph break alongside `\par`, `\'XX` hex escapes decoded via the document's declared `\ansicpg` code page, `\ucN` fallback-byte skipping, `\field`/`HYPERLINK` flattened to plain text, layout-only control words including `\deftab`/`\pardeftab` ignored silently — no RTF-parsing dependency added), `binder-mapper.ts` (maps the Binder tree to a folder/resource creation plan: Draft content lands at the project root, Research text and every other user-created top-level folder each become a top-level GetWrite folder; excludes `TrashFolder`, any `Type="Other"` item, and Research media/snapshots, recording all three plus non-text Research content for the report; a title-less binder item gets a per-parent "Untitled"/"Untitled 2" fallback name, also reported), `metadata-mapper.ts` (maps Status/Label/Keywords/CustomMetaData onto GetWrite's `config.statuses`, a "Label" select field, tags, and per-project custom metadata-schema fields, merging same-leaf-name Keywords into one tag; a candidate field key colliding with a built-in is retried as `<key>-scrivener`, `<key>-scrivener-2`, ... until free, keeping the Scrivener title as the label, with the rename reported — a per-field schema failure is a skip, never an abort), `apply-document-metadata.ts` (per-resource synopsis/notes sidecar merge plus the aggregated per-project feature-toggle enablement this implies), `import-report.ts` (`buildImportReport`/`writeImportReport` — renders and persists the fixed eight-section post-import report — Skipped Items, Field Key Renames, Keyword Merges, Unconverted Research Content, Excluded "Other" Items, Trash Content, Snapshot History, Untitled Fallback Names — to `<projectRoot>/scrivener-import-report.txt`), `import-scrivener-project.ts` (`importScrivenerProject`, the single orchestrating entry point wiring the above into one destination-project creation — refusing a non-empty pre-existing `projectRoot` up front and deleting a run-created `projectRoot` on a fatal mid-write error — with background indexing and the backlinks watcher suspended for the whole write phase (`withIndexingSuspended`, `indexer-queue.ts`) and a from-scratch index/backlinks/mention rebuild mirroring the `reindex` CLI as the last step; throws `UnsupportedScrivenerProjectError` for the FR-2 refusal case and `DestinationNotEmptyError` for the non-empty-destination case, both before any destination write)
...
- Build flow: `pnpm electron:build` (frontend `next build` → electron `tsc` → electron `build:worker`); package with `pnpm electron:package` (electron-builder).
- Logs go to `app.getPath("logs")/getwrite.log` in production.
- **Scrivener import UI (Feature 43)**: `main.ts` registers two IPC channels — `getwrite:scrivener-choose-source` (native picker, returns an opaque selection handle + display name via `scrivener-import/selection-handles.ts`; the source path itself never crosses into the renderer) and `getwrite:scrivener-start-import` (guarded to one in-flight import at a time by `scrivener-import/import-guard.ts`) — exposed on the desktop bridge as `chooseScrivenerSource`/`startScrivenerImport` (`preload.ts`, `frontend/src/lib/desktop-bridge.ts`). Starting an import forks a second, separate utility process — `electron/worker/scrivener-import-worker.ts`, bundled by esbuild (`build:worker`) to `electron/dist/scrivener-import-worker.cjs` and shipped in packaged builds via `electron-builder.yml`'s `extraResources` (`utilityProcess.fork` cannot fork a script packed inside `app.asar`, the same reason the Next server itself ships as `extraResources`) — which runs the existing `importScrivenerProject` pipeline (`frontend/src/lib/models/scrivener/`, imported through the `@gw/core` esbuild alias) and reports back one of four discriminated outcomes (`scrivener-import/handle-import-request.ts`, awaited by `scrivener-import/await-worker-outcome.ts`). The UI lives in `frontend/components/Start/ImportScrivenerDialog.tsx`, opened from an "Import" button on `StartPage.tsx` that only renders when a desktop bridge is present.
...
- `getwrite-cli project import-scrivener <scrivPath> [projectRoot] [-n <name>]` — Import a Scrivener 3, Mac-authored `.scriv` project into a new, complete GetWrite project (Feature 31; delegates to `models/scrivener/import-scrivener-project.ts`). The desktop app exposes the same pipeline as an "Import" flow on the Start Page (Feature 43 — see the Electron Shell section above), rather than only through this CLI command. Refuses — before writing anything — a source whose `.scrivx` `Creator` attribute doesn't start with `SCRMAC-3` (Scrivener 2 and Windows-authored projects are refused by this allow-list, not specifically detected) or a `projectRoot` that already exists and is non-empty. Carries over the Draft/Research/other-top-level-folder binder hierarchy (title-less items get an "Untitled"/"Untitled 2" fallback name), each document's text (RTF → TipTap, bold/italic preserved), and synopsis/notes/status/label/keywords/custom-metadata onto GetWrite's metadata layer (a field-key clash with a built-in is resolved with a suffixed key). Skips (and reports, never aborts on) unconvertible RTF features, malformed `.scrivx` fragments, the project's Trash, non-text Research content, and any `Type="Other"` binder item; writes a `scrivener-import-report.txt` at the destination project root with eight fixed sections (skips, field-key renames, keyword-tag merges, unconverted Research content, excluded `Other` items, Trash content, snapshot history, untitled fallback names).
...
- **Project type** — A JSON spec under `getwrite-config/templates/project-types/` describing folder layout, default metadata fields, and resource templates for a new project (e.g. `novel_project_type.json`). May also seed a project's initial `statuses` and `relationshipTypes` lists (both optional arrays on the spec, copied into the new project's config on creation by `project-creator.ts`).
...
- **Entity** — A resource explicitly declared as such via its sidecar's `entityKind` field (an open, user-definable string like `"character"` or `"place"`, not an enum) plus an optional `aliases` array of alternate names. Declared in `EntitySidecarFieldsSchema` (`schemas.ts`). The sidebar's Remove Entity control (`RemoveEntityControl.tsx`) reverses this declaration by clearing both `entityKind` and `aliases`, with an optional opt-in to also delete the entity's authored relationship edges (see Entity relationship, below); the resource itself is never deleted. `EntitySection.tsx` also lets a writer clear just the Entity Kind field directly, independent of Remove Entity: that path clears only `entityKind` and leaves `aliases` in place but dormant (unmatched, since matching requires a declared entity). Both paths persist to disk via `updateSidecar`'s `clearKeys` parameter (`resource-crud-core.ts`'s `updateSidecarCore`) rather than an `undefined`-valued key in the update body, which a prior version relied on and which never survived the HTTP request.
...
- **Entity highlighting** — A view-layer-only decoration in the editor (`EntityHighlightDecorationExtension.ts` / `entityHighlightDecoration.ts`) that visually marks an entity's name/alias occurrences in the live, unsaved ProseMirror document as one of two states — plain match, or "needs attention" (ambiguous claim and/or a short/common-word alias). Distinct from a Mention: it never reads or writes the mention index or any persisted state, and it produces no data of its own — only CSS decorations recomputed on each document-changing transaction. Gated behind the `entityHighlighting` project feature flag. `entityHighlighting` and `entities` are independent optional booleans in the schema (`ProjectFeatureFlagsSchema`) with no schema-level dependency between them; the "highlighting needs entities on" relationship is enforced by convention at three separate points — `ProjectFeatureToggles.tsx` only renders the `entityHighlighting` toggle once `entities` is on, `TipTapEditor.tsx` re-checks both flags at the point of effect, and the spec states the dependency — not by the schema itself.
- **Entity roster** — The project-wide, read-only sixth Work Area view (`EntityRosterView.tsx` / `EntityRosterRow.tsx`, "Entities" tab) listing every declared entity with its `entityKind`, aliases, mention count and the number of resources those mentions span (both shown, since a resource total hides how heavily an entity features and a mention total hides how widely it is spread), and the same ambiguous-alias / common-word "needs attention" state entity highlighting surfaces. Rides the existing `entities` feature flag and adds none of its own. Distinct from a Mention or the mention index: it is a view over already-computed data (the cached `EntityAliasTable` plus a one-shot `getProjectMentionCounts` read, `mentions-core.ts`) and persists nothing itself. Activating a row selects that entity's resource and switches back to the edit view, where `EntitySection` remains the place to actually edit an entity's aliases.
- **Entity-scoped compile** — Compiling every resource associated with a declared entity — the same merged Mention + Backlinks set `EntityMentionsSection.tsx` renders — into a single ordered document, ordered by resource-tree position (depth-first, siblings by `orderIndex`; `orderResourceIdsByTreePosition` in `compileSelection.ts`). Triggered from `EntityCompileSection.tsx` in the entity's own sidebar view via `CompilePreviewModal.tsx`'s entity mode, distinct from the whole-project/subtree compile flow in `AppShell.tsx`. `EntityCompileSection` renders outside the collapsible "Entity Mentions" section deliberately, so collapsing that section does not hide the compile action; it shares one `getEntityMentionedIn` fetch with `EntityMentionsSection.tsx` through `EntityMentionsContext.tsx`, which owns that fetch. Export-only: it reuses the existing compile client functions and writes nothing.
- **Entity co-occurrence** — A derived, per-project observation that two declared entities were each detected as mentioned somewhere in the same resource (`getEntityCooccurrence`, `mentions-core.ts`). It is computed only from the mention index's `MentionRecord`s grouped by their shared `resourceId` — never from `backlinks.json` and never merged with `getEntityMentionedIn`'s output — so an explicit link with no corresponding detected mention does not count, and it is not conflated with the Mention+Backlinks set `EntityMentionsSection.tsx` otherwise renders. Same-resource is the whole unit of proximity; a mention's character offset within the resource is not consulted. It makes no claim of an authored relationship between the two entities, only that their names or aliases both surfaced in the same resource's prose — rendered as plain text ("Also appears with: …"), not the "Linked"/"Mentioned" badge markup a real relationship would use. Pairs are unordered and non-reflexive: if A co-occurs with B, B's entry lists A with the identical shared-resource count and ids, and an entity is never paired with itself. An entity sharing no resource with any other declared entity's mention is omitted entirely, not listed with a zero count.
- **Entity relationship** — An authored, directed, typed edge between two declared entities, persisted at `meta/relationships.json` (`entity-relationships.ts`: `loadEntityRelationships`/`createEntityRelationship`/`removeEntityRelationship`/`removeEntityRelationshipsForEntity`), surfaced in an entity's own sidebar view by `EntityRelationshipsSection.tsx`. It is the fourth category alongside Mention, Backlinks, and Entity co-occurrence, and is distinct from each: unlike a Mention, it is authored, never detected or inferred; unlike a Backlink (also authored), it is entity-to-entity and typed and directed rather than resource-to-resource and untyped; unlike Entity co-occurrence, it is asserted rather than derived from shared prose proximity. `createEntityRelationship` is idempotent on the `(sourceEntityId, targetEntityId, relationshipType)` triple — a repeat call with the identical triple returns the existing edge rather than duplicating it — and rejects a same-entity edge (`SameEntityRelationshipError`) and a `relationshipType` outside the project's configured list (`InvalidRelationshipTypeError`). No inverse edge is created automatically; each direction must be authored separately. Relationship types come from a per-project extensible `relationshipTypes` list on the project config (`ProjectConfigSchema`), optionally seeded by a project type's own `relationshipTypes` array — the same shape as `statuses`. Deleting an entity's resource never touches its edges, mirroring how `backlinks.json` and `mentions.json` are left untouched on delete. The sidebar's Remove Entity control (`RemoveEntityControl.tsx`), which un-declares the entity without deleting the resource, also leaves edges untouched by default, and only deletes every edge naming the entity (via `removeEntityRelationshipsForEntity`) when the writer ticks an unchecked-by-default checkbox, shown only when at least one edge names the entity. When it does delete edges, it calls `notifyRelationshipsChanged()` on `EntityRelationshipsRefreshContext.tsx` — a narrow, write-signalling sibling to `EntityMentionsContext.tsx` that shares only a monotonic refresh token, never data — so `EntityRelationshipsSection.tsx` re-runs its own independent fetch without a remount. `EntityRelationshipsSection.tsx` renders a "Unknown entity" placeholder in place of a dangling edge's other-entity name rather than hiding, filtering, or crashing.
- **Entity relationship graph** — The project-wide, read-only seventh Work Area view (`EntityRelationshipGraphView.tsx`, "Graph" tab) that draws every declared entity as a node and layers two edge kinds on top: derived Entity co-occurrence edges and authored Entity relationship edges, visually distinguished and not by colour alone — co-occurrence renders dashed and undirected with stroke thickness encoding shared-resource count, authored renders solid and directed with an arrowhead (`EntityGraphCanvas.tsx`, a hand-rolled SVG driven by a `d3-force` simulation for layout, with pan/zoom/selection). It ships a synchronized accessible list (`EntityGraphAccessibleList.tsx`) as its accessibility surface, generalizing the roster's semantic-list pattern — the same node and edge data rendered as a `<ul>` of activatable entity buttons plus a literal-text description of every edge, since thickness and arrowheads are not perceivable through a screen reader. Each edge's disclosure text is composed by `describeCooccurrenceEdge`/`describeAuthoredEdge` in the shared `edgeDescriptions.ts` module (not defined in `EntityGraphAccessibleList.tsx` itself), so the accessible list's wording and the canvas's own hover/tap edge tooltip can never drift apart — hovering (mouse) or tapping (touch) a transparent 12px hit-target rendered along each edge shows the identical text in a bespoke overlay (`entityGraphTooltipOverlay.css`, following the `RefHoverPreview.tsx` pattern rather than the shared `HoverTip.tsx` primitive, since hover and tap on one anchor need open/close behavior `HoverTip.tsx` doesn't support); the overlay is placed by `edgeTooltipPlacement.ts` beside the pointer, fully on-screen, on whichever diagonal side covers the least of the hovered edge's two endpoint nodes, and wraps at the shared tooltip's `min(260px, calc(100vw - 24px))` width cap; edges themselves are not keyboard-focusable and show no tooltip on focus, so the accessible list remains the only keyboard/screen-reader route to that text. It assembles its graph data (`EntityRelationshipGraphView.tsx`) from the same already-`createTransport`-collapsed reads the roster and an entity's own sidebar view already use — the cached `EntityAliasTable`, `getEntityCooccurrence`, and `listEntityRelationships` — adding no new transport code of its own. Rides the existing `entities` feature flag and adds none of its own. It is read-only: edge authoring stays on Feature 38's `EntityRelationshipsSection.tsx` sidebar surface, not here. Node positions are ephemeral, computed fresh by the force simulation on each mount and never persisted to `localStorage`, a sidecar, or any API. A writer can also drag a node to reposition it (entity-graph-node-dragging, FR-40): dragging is a static reposition only — no simulation re-runs during or after a drag, no other node moves, and `computeGraphLayout` itself is unchanged (still stopped immediately and advanced a fixed 300 synchronous ticks), so a dragged edge stretches toward its new endpoint rather than the layout relaxing around it. Dragging works for mouse, pen, and touch: it runs on Pointer Events, and a native non-passive `touchmove` listener prevents scrolling while a node drag is active, because on an Android WebView `touch-action: none` on the node's SVG `<g>` alone did not stop the browser claiming the gesture for scrolling; a swipe that starts on empty canvas still scrolls the pane. A drag's position lives only in in-memory component state (`nodePositionOverrides`, keyed by `entityId`) alongside the existing `selectedNodeId`/`pan`/`scale` state and is exactly as ephemeral as the simulation's own placement — it does not survive a reload or a view switch, and a stale override for a since-deleted entity is left un-purged and inert rather than cleaned up. A pointer gesture below a small movement threshold still behaves as a click (select + navigate); above it, it repositions instead — the threshold is a starting value that was hand-checked once and judged tolerable, not a measured one. There is no keyboard-operable equivalent for dragging; keyboard users keep full read access via the accessible list but cannot reposition a node. Activating a node (canvas or list) selects that entity's resource and switches back to the edit view, mirroring the roster's `onEntityActivated` pattern.
