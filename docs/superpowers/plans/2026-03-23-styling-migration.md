...
| `shadow-*` | *(delete, no replacement)* | No drop shadows — STYLING.md S6 |
...
```css
    ...
    .start-page-shell {
        ...
        background-color: var(--color-gw-editor); /* warm surface for writing context */
...
- [ ] **Step 1: Add workarea classes to getwrite-utilities.css**
...
- [ ] Type a paragraph. Confirm it renders in IBM Plex Serif at 15px with 1.8 line height.
- [ ] Apply **Bold** (Ctrl/Cmd+B) to selected text. Confirm text is bold. Bold must survive — not be overridden by the `.tiptap p` rule.
- [ ] Apply *Italic* (Ctrl/Cmd+I) to selected text. Confirm text is italic.
- [ ] Apply Underline (Ctrl/Cmd+U). Confirm underline appears.
- [ ] Apply a highlight color via the toolbar. Confirm the highlight appears.
- [ ] Change font family to Domine (or any available font). Confirm the font changes.
- [ ] Change font size (increase/decrease). Confirm the size changes.
...
Start dev server. Test:
- [ ] Start page loads, correct brand colors, no shadows, no gradients
- [ ] Open a project — AppShell renders with correct chrome/editor surface temperature difference
- [ ] Light/dark mode toggle — both modes look correct
- [ ] Resource tree hover/active states use brand colors
- [ ] Modals (create resource, project types, help) open with flat bordered surfaces
- [ ] Editor toolbar — no blue active states
- [ ] Type in editor — IBM Plex Serif at 15px, 1.8 line height
- [ ] Run manual editor formatting test (from Task 12 Step 3)
