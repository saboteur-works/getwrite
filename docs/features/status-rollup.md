...
- `frontend/components/WorkArea/StatusRollup.tsx` - renders a table (`<caption>` is screen-reader-only, column headers `scope="col"`, status cells `<th scope="row">`); all user-visible strings live in `STATUS_ROLLUP_COPY`. The table is replaced by "No text resources yet." when no text resource exists, so the "No status" row is present only when at least one text resource exists. The scope note and stale note render in every state; the no-statuses hint renders when `config.statuses` is empty. No red is used.
...
- Word count uses the same source as DataView (`userMetadata.wordCount ?? wordCount ?? 0`), so it can read low for resources with older plain-text revisions. The stale note reads "Word counts for some older resources may read low." (user-confirmed wording). The no-statuses hint ("No statuses are configured for this project, so every resource shows under No status.") no longer references Project Settings.
