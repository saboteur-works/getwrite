...
- Footer: `frontend/components/WorkArea/WritingLogFooterDisplay.tsx`. Collapsed by default, it shows "Today: N / goal" (or "Today: N" with no goal); expanded it adds Added, Deleted, Net and a separate "Imported (not counted toward goal)" figure. A failed read shows "Today's count is unavailable", not zero. The expanded state persists in `localStorage` under `getwrite.editFooter.expanded` (`frontend/src/lib/edit-footer-state.ts`). The display is not red.
