...
**Status:** ✅ Complete (2026-06-14) — Decoupled the card from any data source: `OrganizerCard` now takes a resolved `body?: string` prop (and no longer reads `resource.userMetadata?.notes` — "no card reads notes directly" holds); the body section still renders only under `showBody && body`, so the `showBody` toggle is unchanged. New pure module `OrganizerView/cardBody.ts` exports `resolveOrganizerCardBody(resource, config, { notesEnabled })` (+ `makeTextExcerpt`, `DEFAULT_CARD_EXCERPT_LENGTH = 200`): `none` → no body; `field` → the configured `userMetadata[fieldKey]` formatted to one string (strings trimmed/empty→none, numbers/booleans stringified, arrays comma-joined, structured refs/objects → none); `text-excerpt` → `resource.plainText` truncated to `excerptLength` (defaulting to 200 when unset/≤0) with a trailing ellipsis when clipped. **Back-compat:** when `config` is `null` (unconfigured project) it mirrors the old behavior — the Notes field when `selectNotesEnabled`, else nothing — matching the Task 6 settings preview default. `OrganizerView` reads `selectActiveProjectOrganizerCardBody` + `selectNotesEnabled` and computes `body` per child. **Known gap (logged):** store resources don't carry `plainText` (the `getLocalResources` load path attaches only `wordCount`), and `previews.ts` has zero callers, so `text-excerpt` mode renders empty in the live app today — the consumption layer is complete/tested but the preview data pipeline (save-path generation + read API/transport + store hydration) is deferred per the RISK note (see `follow-up-work.md` 2026-06-14). Tests: new `tests/organizerCardBody.test.ts` (12 cases — `makeTextExcerpt`, all three modes incl. array join / empty-field / no-fieldKey / cap fallback / no-text, and both back-compat branches); `tests/organizerCard.test.tsx` updated to the `body` prop (+ a no-body case); `tests/organizerView.test.tsx` gains 4 store-driven integration cases (field renders the metadata value + notes no longer leaks; text-excerpt renders the capped excerpt; none renders no body; unconfigured+Notes-on falls back to the Notes field). `OrganizerCard` Default story now passes a sample `body`. Verified: organizer suites 23/23 green; full `pnpm test:ci` 170/171 files (1913 passed, 1 skipped) — the 2 failures are the pre-existing `media-file-route` `ENOTEMPTY` teardown flake (passes 5/5 in isolation) + the `revision-manager` worker-teardown flake; `pnpm typecheck` clean; changed files eslint-clean apart from the file's established store-dispatch `as any` casts and one pre-existing `(resource as any).title`.
