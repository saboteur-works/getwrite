...
import { saveResourceTemplate, loadResourceTemplate } from "@gw/core";
...
test("version creates snapshot and history lists it", async () => {
  ...
  try {
    ...
    const code = await main(["node", "templates.ts", "version", tmp, tpl.id]);
    expect(code).toBe(0);

    const hist = await main(["node", "templates.ts", "history", tmp, tpl.id]);
    expect(hist).toBe(0);
