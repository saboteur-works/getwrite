...
async function main(argv: Argv): Promise<number> {
  ...
  try {
    ...
    if (cmd === "save-from-resource") {
      ...
      console.log(`Saved template ${templateId} from resource ${resourceId}`);
      return 0;
    }
    ...
    if (cmd === "create") {
      ...
      const result = await createResourceFromTemplate(projectRoot, templateId, {
        name,
      ...
      if (dry && (result as any).plannedWrites) {
        ...
        for (const p of (result as any).plannedWrites) {
          console.log(
            `${p.path}` + (p.content === null ? " (no content)" : ""),
          );
        }
        return 0;
      }
