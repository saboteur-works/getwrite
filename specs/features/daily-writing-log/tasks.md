...
**Files:** frontend/components/Layout/ProjectSettingsDialog.tsx, frontend/components/Layout/DailyWordGoalField.tsx (only if needed), frontend/tests/projectSettingsDialog.test.tsx (the 5-tab assertion at :88-101), frontend/tests/projectSettingsDialogDailyGoal.test.tsx (:65), frontend/tests/appShellDailyGoalProjectId.test.tsx (:86), frontend/tests/appShellDailyGoalFooterRefresh.test.tsx (:84), frontend/tests/dailyWordGoalField.test.tsx, frontend/stories/Layout/ProjectSettingsDialog.stories.tsx (:86 comment, :91 "All five tabs"), docs/user/writing-log.md (:18), docs/features/writing-log.md (:59), CLAUDE.md (Glossary "Daily word goal", :269)
**Done when:** read the dialog's tab structure and its story args first; tests written first and observed FAILING on current code, then passing: the dialog has a "Writing Goals" tab, positioned last after Metadata, containing the daily goal field, and the Default Revision Name tab does not; the existing 5-tab count test is updated from five to six tabs only (that one assertion, plus the tab-name expectation if it lists names; nothing else in that test changes), the edit listed in Notes; the story comments/text at stories/Layout/ProjectSettingsDialog.stories.tsx :86 and :91 ("All five tabs") say six tabs; the Task 19 test tests/appShellDailyGoalFooterRefresh.test.tsx (~:84) opens the new tab; every test, story, doc, CLAUDE.md and QA-procedure reference to the old placement is updated and listed in Notes (the two AppShell wiring tests open the new tab instead of Default Revision Name; the Task 16 AppShell-level test, using a project whose `id` differs from its directory basename, still passes through the new path); keyboard and axe: the tab is reachable by keyboard, has an accessible name "Writing Goals", and an axe check on the dialog with that tab selected passes; no behaviour change to saving (status text, validation messages, transport calls) beyond the field's location, asserted by the existing field tests passing unchanged apart from tab navigation; a grep of docs/, CLAUDE.md, frontend/tests and frontend/stories for "Default Revision Name" near "goal" finds no stale claim; the stale-AppShell-config caveat (field prefill stale until project reload) is kept as is; no Feature 61 content added; `pnpm typecheck` and `pnpm lint` clean.
**Depends on:** 19, 11, 16
**Estimate:** 3
...
**Files:** frontend/src/lib/models/revision-core.ts (`logCanonicalSave` ~:382), frontend/tests/unit/revision-writing-log.test.ts, docs/features/writing-log.md (:24, :25), docs/user/writing-log.md, CLAUDE.md (Glossary "Writing log", :268)
**Done when:** tests written first and observed FAILING on current code, then passing: a canonical save with identical words appends no entry and returns no `writingLog` signal (no skip, no marker); a save with `added` > 0 or `deleted` > 0 appends exactly one entry, including a same-size replacement with `net` 0 (for example "a b c" to "a b d" logs added 1, deleted 1, net 0); a save with unreadable or non-TipTap previous content still appends the marker and returns the skipped signal (so the toast path is unchanged); the existing tests in tests/unit/revision-writing-log.test.ts still pass (its "appends exactly one entry" test uses added 2, deleted 1 and is not affected); read every test that saves through `updateRevisionInPlace` or `patchRevisionContent` and reads the log (tests/unit/revision-writing-log.test.ts, tests/unit/writing-log-signal.test.ts, tests/integration/docx-import.test.ts, tests/integration/scrivener-import.test.ts) and list in Notes each one whose assertion changes and why, editing ONLY those; docs/features/writing-log.md, docs/user/writing-log.md and the CLAUDE.md glossary "Writing log" entry drop the "a no-change save still adds an entry of zero" statements and state the new rule, keeping the zero-word IMPORT statement as is (OQ-24 resolved: imports unchanged); a test (in tests/integration/docx-import.test.ts and/or tests/integration/scrivener-import.test.ts) that a zero-word import still writes its one entry (added 0, source-tagged), and a check that the zero-word-import line in docs/features/writing-log.md remains; a grep of docs/, CLAUDE.md, frontend/tests, frontend/stories and specs/features/daily-writing-log* for "0/0/0", "zero-change", "entry of zero" and "no-change" finds no stale claim; `pnpm typecheck`, `pnpm lint` and the full test run are green and `pnpm knip` output is unchanged.
**Depends on:** 5, 20
**Estimate:** 2
...
## Summary
- Total tasks: 21
- Total estimated effort: 67 points (was 65; Task 21 +2, appended 2026-09-26, Amended at Gate 6 by the user, to skip zero-change canonical autosaves; earlier: was 62; Task 20 +3, appended 2026-09-26, Amended at Gate 6 by the user, to move the daily goal into a Writing Goals tab; earlier: was 59; Task 19 +3, appended 2026-09-26 to refresh the footer goal figure after a goal change; earlier: was 51; Task 17 +5 and Task 18 +3, appended at Gate 6 to replace the inline expandable footer with a writing-details overlay; earlier: Task 2 +1, Task 9 +2, Task 16 +2)
...
- Task 20 needs 19, 11 and 16 and runs sequentially after Task 19 (same goal wiring); it is not on the critical path.
- Task 21 needs 5 and 20; it edits `logCanonicalSave` and three doc files that Task 20 also touched, so it runs after Task 20 and is not on the critical path.
...
OQ-14 (the local-day window) was resolved at Gate 4 by the user ("take your recs"). OQ-15 to OQ-19 (the writing-details overlay) were resolved at Gate 6 by the user ("Take your recs"), 2026-09-26. OQ-23 (skip zero-change canonical saves) and OQ-24 (imports unchanged; a zero-word import still writes its entry) were resolved at Gate 6 by the user ("Both, take your recs"), 2026-09-26; see the source spec for each resolution.
