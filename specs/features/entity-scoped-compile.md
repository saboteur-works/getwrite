...
FR-1: Users MUST be able to trigger a compile of the currently-selected declared entity's associated resources from the entity's own panel (`EntityCompileSection.tsx`, which renders only when the selected resource has `entityKind` set — originally part of `EntityMentionsSection.tsx`, split out on 2026-09-08 so the trigger sits outside the collapsible "Entity Mentions" section and is not hidden when that section is collapsed; both read one shared fetch via `EntityMentionsContext.tsx`), reachable only when the project's `entities` feature flag is enabled. The triggering UI MUST reuse `CompilePreviewModal.tsx`'s existing output-option controls (format `Select`, headers checkbox, name input) and MUST replace its user-editable checkbox selection tree with the FR-11 read-only ordered resource list; whether this is implemented as a new component or a prop-driven mode within the existing modal is an implementation choice, not a spec-level one. [US-1]
