...
**Task 7 is NOT complete.** Four of its six items were verified mechanically and
are recorded below. Items 2 and 6 require a human hand and remain outstanding —
the task stays `[ ]` until they are done.
...
| 2 | A small hand wobble does not misfire as a drag | **NOT VERIFIED — needs a human hand** |
...
| 6 | Threshold tuning if 2 or 3 felt wrong | **NOT DONE — depends on item 2** |
...
Items 2 and 6 are the reason this task exists. Item 2 asks whether a movement
a writer would describe as "just clicking, my hand wobbled a little" is
misclassified as a drag at the shipped threshold. That is a judgment about
physical hand movement; a scripted pointer can move exactly 3px or exactly 5px
on demand and learns nothing from doing so. Item 6 is the tuning that follows
from item 2's answer.

`DRAG_CLICK_THRESHOLD_PX = 4` therefore remains what the code comment says it
is: an unverified starting point. Nothing here is evidence that 4 is correct.
