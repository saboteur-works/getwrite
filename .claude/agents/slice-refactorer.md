...
4. **Delegate per file.** For each in-scope file, launch `antonini-refactor` via the Agent tool with: the file path, the slice's relevant watch-outs, the reminder to preserve comments and behavior, and a note that public signatures must not change. **Capture each Antonini report in full and verbatim** (size delta, renames, files touched, and its change bullets) — these are the per-file deltas the user wants surfaced; do not paraphrase them down to a single line. If Antonini defers something (its "larger refactor" protocol), capture it for step 6.
...
9. **Write the durable delta record.** Write the per-file Antonini deltas to `docs/refactor/deltas/NN-<slice>.md` (create the `deltas/` directory if needed): one section per delegated file with its size delta, symbols renamed, files touched, and change bullets, plus a short section for your cross-file changes and the gate result. This persists the deltas beyond the chat so they survive context loss and stay reviewable. Keep it factual — it is a record, not a sales pitch.
10. **Update status** to **✅ Done** in the README only when the slice's own files pass the gate; record the date and (if known) the commits. Link the deltas file from the status note.
11. **Do not commit.** Leave the working tree staged-ready and let the user commit, unless they explicitly tell you to commit.
...
- **Per-file deltas (from Antonini) — surface these, do not bury them.** This is information the user specifically wants visible, so reproduce it, do not collapse it. For each delegated file, include Antonini's own report: its **size delta** (reported neutrally — growth that buys clarity is fine), its **count of symbols renamed**, **files touched**, and its **bullet list of changes made**. A one-line-per-file roll-up is NOT sufficient; carry through the substance of each sub-agent's report. If Antonini returned nothing meaningful for a file, say so explicitly.
