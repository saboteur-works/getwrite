  ...
  return {
    ...
    default: ({
      ...
      <textarea
        ...
        value={value}
        onChange={(e) =>
          onChange && onChange(e.target.value, { type: "doc", content: [] })
        }
      />
    ),
  };
...
describe("EditView", () => {
  it("shows autosave unavailable and unsaved warning when editing a non-canonical revision", async () => {
    ...
    expect(
      screen.getByText(/Autosave unavailable for non-canonical revisions/i),
    ).toBeInTheDocument();
  ...
  it("renders as a height-constrained flex column with the footer as the last direct child", () => {
    const store = makeStore();
    render(
      <Provider store={store}>
        <EditView initialContent="Hello world" />
      </Provider>,
    );

    const shell = screen.getByTestId("editview-shell");

    // Shell must be a flex column that fills its parent's height so the
    // footer can be pinned at the bottom and the editor can scroll in between.
    ...
    // Footer must be the final direct child of the shell — guarantees it
    // sits at the bottom of the flex column.
    const footer = document.getElementById("editview-footer");
