...
describe("MultiResourceRefInput", () => {
  ...
  it("a chip with a non-null id renders a clickable label button", () => {
    renderComponent({ value: [{ id: "id-alice", name: "Alice" }] });
    expect(screen.getByRole("button", { name: "Alice" })).toBeTruthy();
  });

  it("a chip with a null id does not render a clickable label button", () => {
    renderComponent({ value: [{ id: null, name: "Unknown" }] });
  ...
  it("clicking a suggestion is blocked when maxSelections cap is reached", () => {
    ...
    fireEvent.keyDown(screen.getByLabelText("multi-resource-ref-input"), {
      key: "Enter",
    });
    expect(onChange).not.toHaveBeenCalled();
  });
