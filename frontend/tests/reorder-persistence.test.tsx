  ...
  it("persists folder.json and resource sidecar orderIndex after drag/drop", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, opts?: any) => {
      ...
      for (const ro of resourceOrder) {
        try {
          const existing = await readSidecar(tmp, ro.id).catch(() => null);
    ...
    const knownFolderIds = new Set(created.folders.map((folder) => folder.id));
    ...
    expect(
      new Set(payload.folderOrder.map((item: { id: string }) => item.id)).size,
    ).toBe(payload.folderOrder.length);
    expect(
      new Set(payload.resourceOrder.map((item: { id: string }) => item.id))
        .size,
