/**
 * Integration tests for the multi-resource PDF compile route.
 *
...
import { POST } from "../../app/api/compile/pdf/route";
...
async function makeProjectsDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gw-pdf-compile-"));
  tmpDirs.push(dir);
  return dir;
}
...
function compileRequest(body: unknown): Request {
  return new Request("http://localhost/api/compile/pdf", {
    method: "POST",
...
describe("POST /api/compile/pdf", () => {
  it("compiles a resource into a downloadable .pdf", async () => {
    ...
    expect(res.headers.get("Content-Disposition")).toContain("my-novel.pdf");
