...
async function writeSidecarDirect(
    ...
    const filePath = path.join(projectRoot, "meta", `resource-${resourceId}.meta.json`);
...
describe("nullifyResourceRefs (Task 8)", () => {
    ...
    it("preserves the name from the stored ResourceRef when nullifying", async () => {
        ...
        try {
            ...
            await writeSidecarDirect(dir, otherResourceId, {
                ...
                name: "Scene Two",
                userMetadata: {
                    pov: { id: deletedId, name: "Bob the Great" },
                },
            });
            ...
            expect((meta?.["pov"] as Record<string, unknown>)?.["name"]).toBe("Bob the Great");
    ...
    it("patches multiple sidecar files in one call", async () => {
        ...
        try {
            ...
            expect(((s1?.["userMetadata"] as Record<string, unknown>)?.["pov"] as Record<string, unknown>)?.["id"]).toBeNull();
            expect(((s2?.["userMetadata"] as Record<string, unknown>)?.["pov"] as Record<string, unknown>)?.["id"]).toBeNull();
            expect(((s3?.["userMetadata"] as Record<string, unknown>)?.["pov"] as Record<string, unknown>)?.["id"]).toBe("other-id");
        } finally {
            await removeDirRetry(dir);
        }
    });
