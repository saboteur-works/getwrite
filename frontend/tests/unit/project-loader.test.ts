...
describe("loadProjectFromDisk", () => {
    it("loads project metadata, folders, and resources from a freshly created project", async () => {
        ...
        try {
            const spec = {
                ...
                defaultResources: [
                    { name: "Getting Started", type: "text" as const, folder: "Workspace", template: "Hello world" },
                ],
            };
            const { folders: createdFolders, resources: createdResources } =
                await createAndAssertProject(spec, { projectRoot: tmp, name: "Basic Project" });
    ...
    it("loads all nested folders for the novel template (3-level nesting)", async () => {
        ...
        try {
            ...
            const { folders: createdFolders } = await createAndAssertProject(specPath, {
                projectRoot: tmp,
                name: "Novel Load Test",
            });
    ...
    it("returns empty folders array without throwing when folders/ directory is missing", async () => {
        ...
        try {
            ...
            const project = { id: "test-nofold", name: "No Folders", slug: "no-folders", createdAt: new Date().toISOString() };
            await fs.writeFile(path.join(tmp, "project.json"), JSON.stringify(project), "utf-8");
    ...
    it("returns empty resources array without throwing when meta/ directory is missing", async () => {
        ...
        try {
            ...
            const project = { id: "test-nometa", name: "No Meta", slug: "no-meta", createdAt: new Date().toISOString() };
            await fs.writeFile(path.join(tmp, "project.json"), JSON.stringify(project), "utf-8");
    ...
    it("create → reopen loop: data loaded from disk matches data returned at creation", async () => {
        ...
        try {
            ...
            const { project: createdProject, folders: createdFolders, resources: createdResources } =
