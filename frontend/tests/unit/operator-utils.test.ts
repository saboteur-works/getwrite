...
describe("operator-utils — buildOperatorStub", () => {
    ...
    it("multiselect: includes-all-of → and([in])", () => {
        const stub = buildOperatorStub("multiselect", "includes-all-of", "statuses");
        expect(stub).toEqual({
            op: "and",
    ...
    it("multiselect: includes-any-of → or([in])", () => {
        const stub = buildOperatorStub("multiselect", "includes-any-of", "statuses");
        expect(stub).toEqual({
            op: "or",
            children: [{ op: "in", field: "statuses", value: "" }],
        });
    ...
    it("multiselect: includes-none-of → not(or([in]))", () => {
        const stub = buildOperatorStub("multiselect", "includes-none-of", "statuses");
        expect(stub).toEqual({
            op: "not",
            child: {
                op: "or",
    ...
    it("all non-divider operators for every type produce a non-null stub", () => {
        for (const type of ALL_TYPES) {
            for (const entry of OPERATORS_BY_TYPE[type]) {
                ...
                expect(stub, `${type}/${entry.value} should produce a stub`).not.toBeNull();
            }
        }
    });
});
