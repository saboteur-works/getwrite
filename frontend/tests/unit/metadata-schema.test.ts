...
const FIELD: MetadataField = { key: "my-field", label: "My Field", type: "text" };
const LOCKED_FIELD: MetadataField = { key: "locked-field", label: "Locked", type: "text", locked: true };
...
describe("addField", () => {
    it("appends a field to an existing group", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        const newField: MetadataField = { key: "new-field", label: "New", type: "number" };
    ...
    it("persists the added field to disk", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        await addField(dir, GROUP_ID, { key: "persisted", label: "Persisted", type: "boolean" });
        const schema = await getSchema(dir);
    ...
    it("throws when the key already exists in the schema", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        await expect(
            addField(dir, GROUP_ID, { key: "my-field", label: "Duplicate", type: "text" }),
    ...
    it("throws when the group does not exist", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        await expect(
            addField(dir, "nonexistent-group", { key: "x", label: "X", type: "text" }),
...
describe("removeField", () => {
    ...
    it("throws when the field does not exist", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        await expect(
            removeField(dir, GROUP_ID, "nonexistent"),
...
describe("reorderFields", () => {
    it("reorders fields in the specified group", async () => {
        const secondField: MetadataField = { key: "second", label: "Second", type: "date" };
    ...
    it("persists the new order to disk", async () => {
        ...
        const schema = await getSchema(dir);
        expect(schema.groups[0].fields.map((f) => f.key)).toEqual(["second", "my-field"]);
    });
...
describe("renameField", () => {
    it("updates the label of an existing field", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        const schema = await renameField(dir, GROUP_ID, "my-field", "Renamed Label");
        const group = schema.groups.find((g) => g.id === GROUP_ID)!;
    ...
    it("can rename a locked field's label", async () => {
        const { dir } = await makeTmpProject(baseSchema(LOCKED_FIELD));
        const schema = await renameField(dir, GROUP_ID, "locked-field", "New Label");
        const field = schema.groups[0].fields.find((f) => f.key === "locked-field")!;
...
describe("updateFieldOptions", () => {
    it("sets options on a select field", async () => {
        const selectField: MetadataField = { key: "status", label: "Status", type: "select" };
        const { dir } = await makeTmpProject({
            groups: [{ id: GROUP_ID, label: "G", fields: [selectField] }],
        });
        const schema = await updateFieldOptions(dir, GROUP_ID, "status", ["Draft", "Final"]);
...
describe("addGroup", () => {
    it("appends a new group", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        const newGroup: MetadataGroup = { id: "new-group", label: "New Group", fields: [] };
    ...
    it("persists the new group to disk", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        const newGroup: MetadataGroup = { id: "disk-group", label: "Disk Group", fields: [] };
...
describe("removeGroup", () => {
    ...
    it("throws when the group does not exist", async () => {
        const { dir } = await makeTmpProject(baseSchema());
        await expect(removeGroup(dir, "ghost-group")).rejects.toThrow(/Group not found/);
    });
});
