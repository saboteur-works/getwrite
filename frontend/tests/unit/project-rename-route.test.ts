/**
 * Integration tests for the project rename route.
 *
...
import { POST } from "../../app/api/project/rename/route";
...
async function makeProjectsDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gw-rename-"));
  tmpDirs.push(dir);
  return dir;
}
...
async function writeProject(
  ...
  const proj = createProject({ name: "old-name" });
  await fs.writeFile(
...
function renameRequest(body: unknown): Request {
  return new Request("http://localhost/api/project/rename", {
    method: "POST",
...
describe("POST /api/project/rename", () => {
  it("renames the project for a valid projectId", async () => {
    ...
    const res = await withProjectsDir(projectsDir, () =>
      POST(renameRequest({ projectId, newName: "new-name" }) as never),
    );
