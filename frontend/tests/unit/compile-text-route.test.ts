/**
 * Integration tests for the multi-resource plain-text compile route.
 *
...
import { POST } from "../../app/api/compile/text/route";
...
async function makeProjectsDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gw-text-compile-"));
  tmpDirs.push(dir);
  return dir;
}
...
function compileRequest(body: unknown): Request {
  return new Request("http://localhost/api/compile/text", {
    method: "POST",
...
describe("POST /api/compile/text", () => {
  it("compiles a resource's plain text with a header when includeHeaders is true", async () => {
    ...
    expect(json.filename).toBe("my-novel.txt");
    expect(json.text).toContain("Chapter One");
    expect(json.text).toContain("hello there");
  });
