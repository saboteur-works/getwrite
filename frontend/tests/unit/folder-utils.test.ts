...
describe("readFolderTree", () => {
  ...
  it("recurses into subdirectories and collects nested descriptors", async () => {
    ...
    try {
      ...
      await fs.mkdir(path.join(tmp, "workspace", "chapters", "chapter-1"), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(tmp, "workspace", "folder.json"),
      ...
      await fs.writeFile(
        path.join(tmp, "workspace", "chapters", "folder.json"),
        JSON.stringify(chapters),
        "utf-8",
      );
      await fs.writeFile(
        path.join(tmp, "workspace", "chapters", "chapter-1", "folder.json"),
        JSON.stringify(chapter1),
        "utf-8",
      );
...
describe("renameFolderById", () => {
  it("returns null when foldersDir does not exist", async () => {
    const result = await renameFolderById(
      "/tmp/__nonexistent_gw__",
      "any-id",
      "New Name",
    );
  ...
  it("returns null when no folder with matching id exists", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tmp, "other", "folder.json"),
        JSON.stringify(descriptor),
        "utf-8",
      );
  ...
  it("updates name in folder.json and returns updated data", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tmp, "folder-1", "folder.json"),
        JSON.stringify(descriptor),
        "utf-8",
      );
      ...
      const written = JSON.parse(
        await fs.readFile(path.join(tmp, "folder-1", "folder.json"), "utf-8"),
      );
  ...
  it("finds folder by id when directory name is slug, not id", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tmp, "my-folder-slug", "folder.json"),
        JSON.stringify(descriptor),
        "utf-8",
      );
  ...
  it("finds and renames a nested folder", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tmp, "parent", "folder.json"),
        JSON.stringify(parent),
        "utf-8",
      );
      await fs.writeFile(
        path.join(tmp, "parent", "child", "folder.json"),
