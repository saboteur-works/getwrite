...
async function makeTmpProjectsDir(): Promise<{
  ...
  const projectsDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "gw-entity-cooccurrence-route-"),
  );
...
describe("GET /api/project/[project-id]/entity-cooccurrence", () => {
  it("returns co-occurrence pairs for a project with a built mention index", async () => {
    ...
    await withProjectsDirEnv(projectsDir, async () => {
      ...
      >;
      // Aria and Brann share resource-1 only; resource-2 has just Aria, so
      // no additional pair is formed there.
      expect(json).toEqual({
        [ariaId]: [
          { entityId: brannId, count: 1, resourceIds: ["resource-1"] },
        ],
        [brannId]: [
          { entityId: ariaId, count: 1, resourceIds: ["resource-1"] },
        ],
      });
    });
  });
