...
describe("computeProximityScore", () => {
    ...
    it("returns 0 when one term is missing entirely", () => {
        expect(computeProximityScore("dragon flies high", ["dragon", "knight"])).toBe(0);
    });
    ...
    it("finds the minimum window across multiple occurrences", () => {
        ...
        const scoreWithClose = computeProximityScore(textWithClose, ["tall", "one"]);
        const scoreOnlyClose = computeProximityScore(textOnlyClose, ["tall", "one"]);
