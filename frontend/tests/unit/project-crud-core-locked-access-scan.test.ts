...
/**
 * @module tests/unit/project-crud-core-locked-access-scan
 *
 * Regression coverage for `findProjectRootByInternalId`
 * (`lib/models/project-crud-core.ts`): the locked-access fail-closed check it
 * runs per scanned entry must be scan-local. A locked project *adjacent* to
 * the target must not abort the whole scan — only a locked project that is
 * itself the target (or a scan that finds no match at all after skipping a
 * locked entry) should throw.
 *
 * Complements `project-crud-core-locked-access.test.ts`, whose cases all
 * place the locked project as the operation's own target.
 */
...
import { findProjectRootByInternalId } from "../../src/lib/models/project-crud-core";
...
const PLAIN_ID = "55555555-5555-4555-8555-555555555555";
...
describe("findProjectRootByInternalId — locked entry adjacent to the scan target", () => {
  it("finds the unencrypted project despite a locked project elsewhere in the workspace", async () => {
    ...
    const result = await runInStorageContext(
      ...
      () => findProjectRootByInternalId(WORKSPACE, PLAIN_ID),
    );
  ...
  it("throws the locked-access error (not null) when no entry matches and a locked entry was skipped", async () => {
    ...
    await expect(
      runInStorageContext({ tenantRoot: WORKSPACE, adapter }, () =>
        findProjectRootByInternalId(WORKSPACE, "no-such-id"),
      ),
  ...
  it("still returns null for a nonexistent id when no project in the workspace is locked", async () => {
    ...
    const result = await runInStorageContext(
      ...
      () => findProjectRootByInternalId(OTHER_WORKSPACE, "no-such-id"),
    );
  ...
  it("still throws when the locked project is itself the target", async () => {
    ...
    await expect(
      runInStorageContext({ tenantRoot: WORKSPACE, adapter }, () =>
        findProjectRootByInternalId(WORKSPACE, SEALED_ID),
      ),
