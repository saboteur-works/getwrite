...
describe("evaluateCached", () => {
    ...
    it("isolates cache entries by queryId within the same project", async () => {
        ...
        let countA = 0;
        let countB = 0;
        await evaluateCached(root, "qA", async () => { countA++; return ["a"]; });
        await evaluateCached(root, "qB", async () => { countB++; return ["b"]; });
        await evaluateCached(root, "qA", async () => { countA++; return ["a"]; });
        await evaluateCached(root, "qB", async () => { countB++; return ["b"]; });
    ...
    it("propagates evaluator errors without caching the result", async () => {
        ...
        await expect(evaluateCached(root, "q-err", evaluator)).rejects.toThrow("evaluator failed");
        expect(callCount).toBe(1);
