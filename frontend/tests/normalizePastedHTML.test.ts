...
describe("normalizePastedHTML", () => {
  ...
  describe("heading inline styles (FR4)", () => {
    ...
    it("strips heading styles from all six heading levels", () => {
      const input = [1, 2, 3, 4, 5, 6]
        .map((n) => `<h${n} style="font-size: 24px; color: red">H${n}</h${n}>`)
        .join("");
      const result = normalizePastedHTML(input);
      expect(result).not.toContain("font-size");
      expect(result).not.toContain("color");
  ...
  // FR7 — table cell attribute and style stripping
  describe("table cell normalization (FR7)", () => {
    it("removes bgcolor attribute from td", () => {
      const input = `<table><tr><td bgcolor="#ffffff">cell</td></tr></table>`;
      const result = normalizePastedHTML(input);
      expect(result).not.toContain("bgcolor");
      expect(result).toContain("cell");
    });

    it("removes bgcolor attribute from th", () => {
      const input = `<table><tr><th bgcolor="#cccccc">header</th></tr></table>`;
      const result = normalizePastedHTML(input);
      expect(result).not.toContain("bgcolor");
      expect(result).toContain("header");
    });

    it("removes width attribute from td", () => {
      const input = `<table><tr><td width="200">cell</td></tr></table>`;
      const result = normalizePastedHTML(input);
      expect(result).not.toMatch(/td[^>]*width=/);
      expect(result).toContain("cell");
    });

    it("removes height attribute from tr", () => {
      const input = `<table><tr height="40"><td>cell</td></tr></table>`;
      const result = normalizePastedHTML(input);
      expect(result).not.toMatch(/tr[^>]*height=/);
    });

    it("removes background shorthand inline style from td", () => {
      const input = `<table><tr><td style="background: white">cell</td></tr></table>`;
      const result = normalizePastedHTML(input);
      expect(result).not.toContain("background");
      expect(result).toContain("cell");
    });

    it("removes width inline style from td", () => {
      const input = `<table><tr><td style="width: 150px">cell</td></tr></table>`;
      const result = normalizePastedHTML(input);
      expect(result).not.toContain("width: 150px");
    });

    it("preserves cell text and table structure after stripping", () => {
      const input = `<table><tr><th bgcolor="#eee" width="100" style="color:black; font-family: Arial">Name</th></tr><tr><td bgcolor="#fff" style="background: white; width: 100px">Gandalf</td></tr></table>`;
      const result = normalizePastedHTML(input);
    ...
    it("does not strip table attributes from internal paste (data-pm-slice)", () => {
      const input = `<meta data-pm-slice="1 1 []"><table><tr><td bgcolor="#fff">cell</td></tr></table>`;
      const result = normalizePastedHTML(input);
