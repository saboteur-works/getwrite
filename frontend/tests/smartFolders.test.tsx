...
describe("SmartFolders", () => {
  ...
  it("renders each saved query as a row", () => {
    renderSmartFolders({ [QUERY_A.id]: QUERY_A, [QUERY_B.id]: QUERY_B });
  ...
  it("calls onEditQuery with the query when edit button is clicked", () => {
    const onEditQuery = vi.fn();
    renderSmartFolders({ [QUERY_A.id]: QUERY_A }, undefined, vi.fn(), {
      onEditQuery,
    });
    fireEvent.click(
      screen.getByRole("button", { name: `Edit ${QUERY_A.name}` }),
    );
  ...
  it("calls onDeleteQuery with the queryId when delete button is clicked", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: `Delete ${QUERY_A.name}` }),
    );
