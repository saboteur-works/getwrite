...
describe("Sidebar Controls", () => {
  ...
  it("POVAutocomplete calls onChange with ResourceRef when name matches", () => {
    ...
    render(
      <POVAutocomplete
        onChange={onChange}
        resourceOptions={[{ id: "uuid-x", name: "X" }]}
      />,
    );
  ...
  it("EditorMenuInput syncs schema-provided values and forwards select changes", async () => {
    ...
    expect(screen.getByRole("combobox", { name: /Font Style/i })).toHaveValue(
      "Georgia",
    );
  ...
  it("DurationInput renders quantity field and unit select", () => {
    ...
    expect(
      screen.getByLabelText("story-duration-quantity"),
    ).toBeInTheDocument();
  ...
  it("DurationInput emits minutes when entering quantity in minutes", () => {
    ...
    fireEvent.change(screen.getByLabelText("story-duration-quantity"), {
      target: { value: "60" },
    });
  ...
  it("DurationInput emits correct minutes when unit is hours", () => {
    const onChange = vi.fn();
    render(<DurationInput onChange={onChange} />);
    fireEvent.change(screen.getByLabelText("story-duration-unit"), {
      target: { value: "hours" },
    });
    fireEvent.change(screen.getByLabelText("story-duration-quantity"), {
      target: { value: "2" },
  ...
  it("DurationInput initialises to 2 hours when value=120 (smart unit detection)", () => {
    ...
    const qty = screen.getByLabelText(
  ...
  it("EditorMenuColorSubmenu opens and forwards the chosen color", async () => {
    ...
    expect(
      screen.queryByRole("menuitemradio", { name: /Select color #111827/i }),
    ).not.toBeInTheDocument();
  });

  it("EditorMenuColorSubmenu does not render a clear tile when onClearColor is omitted", async () => {
    const user = userEvent.setup();
    ...
    expect(
      screen.queryByRole("menuitem", { name: /Clear color/i }),
    ).not.toBeInTheDocument();
  });

  it("EditorMenuColorSubmenu renders and invokes the clear tile when onClearColor is provided", async () => {
    const user = userEvent.setup();
    const onSelectColor = vi.fn();
    ...
    // Menu closes after clearing
    expect(
      screen.queryByRole("menuitem", { name: /Clear text color/i }),
    ).not.toBeInTheDocument();
  });
});
...
describe("EndDateInput", () => {
  ...
  it("changing datetime-local input fires onChange with ISO string", () => {
    ...
    render(
      <EndDateInput
        computedEndDate="2024-06-01T02:00:00.000Z"
        onChange={onChange}
      />,
    );
    fireEvent.click(screen.getByLabelText("end-date-override-toggle"));
  ...
  it("clicking Clear override fires onChange(null) and returns to read-only", () => {
    const onChange = vi.fn();
    render(
      <EndDateInput overrideValue="2024-06-01T04:00" onChange={onChange} />,
    );
