...
describe("a11y: Checkbox primitive", () => {
  ...
  it("renders checked when checked prop is true", () => {
    render(
      <Checkbox aria-label="include headers" checked onChange={vi.fn()} />,
    );
  ...
  it("disabled checkbox is not interactable", async () => {
    const onChange = vi.fn();
    render(
      <Checkbox aria-label="include headers" disabled onChange={onChange} />,
    );
  ...
  it("caller className is merged with base classes", () => {
    render(
      <Checkbox aria-label="include headers" className="w-[13px] h-[13px]" />,
    );
    const cb = screen.getByRole("checkbox");
  ...
  it("forwards aria-label to the checkbox element", () => {
    ...
    expect(
      screen.getByRole("checkbox", { name: "compile all resources" }),
    ).toBeInTheDocument();
  });
});
