...
describe("a11y: CreateProjectModal keyboard navigation (T018)", () => {
    beforeEach(() => {
        vi.restoreAllMocks();
        vi.spyOn(globalThis as any, "fetch").mockImplementation((input: RequestInfo) => {
            const url = typeof input === "string" ? input : (input as Request).url;
            if (url.endsWith("/api/project-types")) {
                return Promise.resolve({ ok: true, json: async () => [
                    { id: "novel", name: "Novel", folders: [{ name: "Workspace" }] }
                ] });
            }
            return Promise.resolve({ ok: true, json: async () => ({}) });
        });
    ...
    it("focuses controls in a logical order and supports Escape to close", async () => {
        ...
        const filter = screen.getByRole("searchbox", { name: /Filter project types/i });
