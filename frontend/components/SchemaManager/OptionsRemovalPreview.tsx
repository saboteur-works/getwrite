...
export default function OptionsRemovalPreview({
    ...
    function setRowAction(option: string, action: ActionChoice): void {
        setRows((prev) => prev.map((r) => r.option === option ? { ...r, action } : r));
    }

    function setRowNormalizedTo(option: string, normalizedTo: string): void {
        setRows((prev) => prev.map((r) => r.option === option ? { ...r, normalizedTo } : r));
    }
    ...
    async function handleApply(): Promise<void> {
        ...
        try {
            await dispatch(updateMetadataFieldOptionsWithMigration({
                projectId,
                groupId,
                fieldKey,
                newOptions,
                    ...
                    {!loading && !fetchError && rows.length > 0 && (
                        <div className="mb-4">
                            <div className="grid grid-cols-[1fr_auto_auto] gap-x-3 gap-y-1 items-center">
                                <span className="font-mono text-[8px] uppercase tracking-[0.14em] text-gw-dim pb-1">Removed option</span>
                                <span className="font-mono text-[8px] uppercase tracking-[0.14em] text-gw-dim pb-1 text-right">Used by</span>
                                <span className="font-mono text-[8px] uppercase tracking-[0.14em] text-gw-dim pb-1">Action</span>

                                {rows.map((row) => (
                                    <React.Fragment key={row.option}>
                                        <span className="font-mono text-[10px] text-gw-primary truncate" title={row.option}>
                                            {row.option}
                                        </span>
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            <select
                                                ...
                                                value={row.action}
                                                onChange={(e) => setRowAction(row.option, e.target.value as ActionChoice)}
                                            ...
                                            {row.action === "normalize" && (
                                                <select
                                                    ...
                                                    onChange={(e) => setRowNormalizedTo(row.option, e.target.value)}
                    ...
                    {!loading && !fetchError && (
                        <div className="border-t border-gw-border pt-3 space-y-1">
                            <p className="font-mono text-[9px] text-gw-secondary">
                                Final options: {finalOptions.length > 0 ? finalOptions.join(", ") : "(none)"}
                            </p>
                            {affectedCount > 0 && (
