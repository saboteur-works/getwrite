 ...
 */
export function getAdaptiveTickStrategy(spanMs: number, zoom: number): TickStrategy {
    const dateOnly: Intl.DateTimeFormatOptions = { month: "short", day: "numeric" };
    const dateTime: Intl.DateTimeFormatOptions = { month: "short", day: "numeric" };
 ...
 */
export function formatTickAdaptive(ms: number, strategy: TickStrategy, locale?: string): string {
    const d = new Date(ms);
    const datePart = new Intl.DateTimeFormat(locale ?? "en-US", strategy.format).format(d);
