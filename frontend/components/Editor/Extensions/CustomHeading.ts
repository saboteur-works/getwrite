...
interface CustomHeadingOptions {
  levels: number[];
  customStyles: { [index in HeadingAttribute]?: Partial<CustomHeadingStyle> };
}
...
const CustomHeading = Heading.extend<CustomHeadingOptions>(() => {
  return {
    ...
    renderHTML({ node, HTMLAttributes }) {
      ...
      if (!style) {
        return [headingAttr, mergeAttributes(HTMLAttributes), 0];
      }
      ...
      return [
        headingAttr,
        styleAttribute
          ? mergeAttributes(HTMLAttributes, { style: styleAttribute })
