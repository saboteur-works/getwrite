...
import { slugifyName, deriveLabel, findExisting, fuzzyMatch } from "../../src/lib/models/field-dedup";
...
export function defaultGroupId(
    ...
    if (currentFolderId) {
        const folderGroup = schema.groups.find((g) => g.folderId === currentFolderId);
        if (folderGroup) return folderGroup.id;
    }
    const firstNonBuiltin = schema.groups.find((g) => !g.id.startsWith("builtin-"));
...
export default function AddFieldForm({
    ...
    const [groupId, setGroupId] = useState(() => defaultGroupId(schema, currentFolderId));
        ...
        try {
            await dispatch(
                addMetadataField({
                    ...
                    field: {
                        key,
                        label: label.trim() || deriveLabel(key),
                        type,
                    },
                }),
            ).unwrap();
                ...
                <div className="mb-2">
                    ...
                    {isExactMatch && (
                        <p className="text-[9px] font-mono text-gw-secondary pl-12 mb-1">
                            Already exists in {exactMatch!.groupLabel} — press Add to go there.
                        </p>
                    )}
                ...
                {!isExactMatch && (
                    <>
                        ...
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            >
                                {(Object.entries(FIELD_TYPE_LABELS) as [MetadataFieldType, string][]).map(([v, lbl]) => (
