              ...
              {index > 0 && (
                <div className="query-builder__pill-row">
                  <button
                    ...
                    onClick={() => onGlobalCombinatorChange?.(nextGlobal)}
                  >
                ...
                <FilterGroup
                  ...
                  onCombinatorChange={
                    isAdvanced
                      ? undefined
                      : (combinator) =>
                          onGroupCombinatorChange?.(group.id, combinator)
                  }
                  onChipUpdate={
                    isAdvanced
                      ? undefined
                      : (chipId, updates) =>
                          onChipUpdate?.(group.id, chipId, updates)
                  }
                  onChipAdd={
                    isAdvanced ? undefined : () => onChipAdd?.(group.id)
                  }
                  onChipDelete={
                    isAdvanced
                      ? undefined
                      : (chipId) => onChipDelete?.(group.id, chipId)
                  }
                  onChipDuplicate={
                    isAdvanced
                      ? undefined
                      : (chipId) => onChipDuplicate?.(group.id, chipId)
                  }
                  onChipReorder={
                    isAdvanced
                      ? undefined
