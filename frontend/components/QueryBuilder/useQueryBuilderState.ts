...
export function useQueryBuilderState(): UseQueryBuilderStateReturn {
  ...
  const onGlobalCombinatorChange = useCallback(
    (combinator: GlobalCombinator) => {
  ...
  const onGroupCombinatorChange = useCallback(
    (groupId: string, combinator: GroupCombinator) => {
      setGroups((prev) =>
  ...
  const onChipReorder = useCallback(
