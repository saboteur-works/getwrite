...
function isGroupNode(node: QueryAST): node is LeafNode | NotNode | AndNode | OrNode {
...
function chipToAst(chip: GroupChip): QueryAST | null {
    ...
    switch (chip.operator) {
        // text
        case "is": return { op: "eq", field: f, value: toMeta(v) };
        case "is-not": return { op: "ne", field: f, value: toMeta(v) };
        case "contains": return { op: "contains", field: f, value: String(v ?? "") };
        case "does-not-contain": return { op: "not", child: { op: "contains", field: f, value: String(v ?? "") } };
        case "starts-with": return { op: "matches", field: f, value: String(v ?? "") };
        case "matches-regex": return { op: "matches", field: f, value: String(v ?? "") };
        // number
        case "equals": return { op: "eq", field: f, value: toMeta(v) };
        case "does-not-equal": return { op: "ne", field: f, value: toMeta(v) };
        case "is-less-than": return { op: "lt", field: f, value: toMeta(v) };
        case "is-at-most": return { op: "lte", field: f, value: toMeta(v) };
        case "is-greater-than": return { op: "gt", field: f, value: toMeta(v) };
        case "is-at-least": return { op: "gte", field: f, value: toMeta(v) };
        case "is-between": {
            const range =
        ...
        // date
        case "is-on": return { op: "eq", field: f, value: toMeta(v) };
        case "is-before": return { op: "lt", field: f, value: toMeta(v) };
        case "is-after": return { op: "gt", field: f, value: toMeta(v) };
        case "in-the-last": return { op: "gte", field: f, value: toMeta(v) };
        // boolean
        case "is-true": return { op: "eq", field: f, value: true };
        case "is-false": return { op: "eq", field: f, value: false };
        // select / resource-ref
        case "is-any-of": return { op: "in", field: f, value: toMeta(v) };
        case "is-none-of": return { op: "not", child: { op: "in", field: f, value: toMeta(v) } };
        // multiselect / multi-resource-ref
        case "includes": return { op: "in", field: f, value: toMeta(v) };
        case "does-not-include": return { op: "not", child: { op: "in", field: f, value: toMeta(v) } };
        case "includes-all-of": return {
            op: "and",
            children: [{ op: "in", field: f, value: toMeta(v) }],
        };
        case "includes-any-of": return {
            op: "or",
            children: [{ op: "in", field: f, value: toMeta(v) }],
        };
        case "includes-none-of": return {
            op: "not",
            child: { op: "or", children: [{ op: "in", field: f, value: toMeta(v) }] },
        };
        // universal
        case "is-empty": return { op: "not", child: { op: "exists", field: f } };
        case "has-any-value": return { op: "exists", field: f };
        default: return null;
    }
}
...
function fieldFor(key: string, fields: FilterChipField[]): FilterChipField | null {
...
function comparisonToOperator(
    ...
    if (astOp === "lt") return fieldType === "date" ? "is-before" : "is-less-than";
    if (astOp === "lte") return "is-at-most";
    if (astOp === "gt") return fieldType === "date" ? "is-after" : "is-greater-than";
    if (astOp === "gte") return fieldType === "number" ? "is-at-least" : "is-after";
    return "is";
}
...
function astNodeToChip(
    ...
    if (node.op === "not") {
        ...
        if (inner.op === "exists") {
            return { id, field: fieldFor(inner.field, fields), operator: "is-empty", value: null };
        }
    ...
    if (node.op === "exists") {
        return { id, field: fieldFor(node.field, fields), operator: "has-any-value", value: null };
    }
    ...
    if (node.op === "in") {
        ...
        const isMulti = field?.type === "multiselect" || field?.type === "multi-resource-ref";
        return {
            id,
            field,
