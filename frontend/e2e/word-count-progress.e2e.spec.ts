...
test("WordCountProgressBar InProgress story renders progressbar", async ({
  page,
}) => {
...
test("WordCountProgressBar InProgress story shows aria values", async ({
  page,
}) => {
  await page.goto("/iframe.html?id=workarea-wordcountprogressbar--in-progress");
  const bar = page.getByRole("progressbar");
...
test("WordCountProgressBar Achieved story shows Goal reached text", async ({
  page,
}) => {
...
test("WordCountProgressBar Empty story renders bar at zero", async ({
  page,
}) => {
