...
test("data view with goal story shows word counts in progress bar", async ({
  page,
}) => {
  await page.goto("/iframe.html?id=workarea-dataview--with-goal");
  const bar = page.getByRole("progressbar");
...
test("data view sorted by last edited shows timestamp labels", async ({
  page,
}) => {
...
test("data view sorted by last edited has sort toggle buttons", async ({
  page,
}) => {
  await page.goto("/iframe.html?id=workarea-dataview--sorted-by-last-edited");
  await expect(
    page.getByRole("button", { name: /last edited/i }),
  ).toBeVisible();
