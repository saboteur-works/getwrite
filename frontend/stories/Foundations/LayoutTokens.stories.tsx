...
const WIDTH_TOKENS: LayoutToken[] = [
  {
    token: "--width-gw-resource-panel",
    px: "200px",
    role: "Resource tree panel width",
  },
  {
    token: "--width-gw-metadata-panel",
    px: "220px",
    role: "Metadata/properties panel width",
  },
];
...
const HEIGHT_TOKENS: LayoutToken[] = [
  ...
  {
    token: "--height-gw-tree-item",
    px: "32px",
    role: "Resource tree row height",
  },
];

const PADDING_TOKENS: LayoutToken[] = [
  {
    token: "--padding-gw-editor-x",
    px: "40px",
    role: "Editor horizontal padding",
  },
...
const COMPONENT_TOKENS: LayoutToken[] = [
  {
    token: "--width-gw-active-indicator",
    px: "2px",
    role: "Left border on active resource tree item",
  },
  {
    token: "--height-gw-tab-underline",
    px: "1.5px",
    role: "Red underline on active editor tab",
  },
];
    ...
    >
      ...
      <div
        style={{
          ...
          backgroundColor: "var(--color-gw-chrome2)",
          borderLeft:
            "var(--width-gw-active-indicator) solid var(--color-gw-red-border)",
          display: "flex",
          alignItems: "center",
          paddingLeft: 8,
          flexShrink: 0,
    ...
    >
      ...
      <div
        style={{
          ...
          paddingBottom: 1,
          borderBottom:
            "var(--height-gw-tab-underline) solid var(--color-gw-red-border)",
          flexShrink: 0,
        }}
      >
        <span
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <section style={{ marginBottom: 48 }}>
        <SectionHeader
          title="Panel Widths"
          description="Width tokens for the three-panel layout. Bars are rendered at the exact token value."
        />
        <div style={{ borderTop: "0.5px solid var(--color-gw-border)" }}>
          {WIDTH_TOKENS.map((t) => (
            <WidthRow key={t.token} {...t} />
          ))}
        </div>
      </section>
      <section style={{ marginBottom: 48 }}>
        <SectionHeader
          title="Heights"
          description="Height tokens for chrome bars. Boxes are rendered at the exact token height."
        />
        <div style={{ borderTop: "0.5px solid var(--color-gw-border)" }}>
          {HEIGHT_TOKENS.map((t) => (
            <HeightRow key={t.token} {...t} />
          ))}
        </div>
      </section>
      <section style={{ marginBottom: 48 }}>
        <SectionHeader
          ...
          description="Padding applied around the writing surface. The dashed border shows the container; the filled block shows the content area."
        />
        <div style={{ borderTop: "0.5px solid var(--color-gw-border)" }}>
          {PADDING_TOKENS.map((t) => (
            <PaddingRow key={t.token} {...t} />
          ))}
        </div>
      </section>
      <section style={{ marginBottom: 48 }}>
        <SectionHeader
          ...
          description="Sub-pixel and small component tokens. Shown in context so their visual effect is legible."
        />
        <div style={{ borderTop: "0.5px solid var(--color-gw-border)" }}>
