...
const DEMOS: FocusDemoItem[] = [
  {
    label: "Button",
    description:
      "box-shadow: 0 0 0 3px var(--color-gw-focus-ring) on :focus-visible",
    element: <ButtonDemo />,
  },
  {
    label: "Input",
    description:
      "Same ring — outline is suppressed, box-shadow used throughout the system",
    element: <InputDemo />,
  },
  {
    label: "Inline link",
...
function FocusDemoRow({
  label,
  description,
  element,
}: FocusDemoItem): JSX.Element {
  return (
    <div
      style={{
        padding: "20px 0",
...
function FocusStatesShowcase(): JSX.Element {
  return (
    <div style={{ maxWidth: 760 }}>
      ...
      >
        All focusable elements use a consistent 3px red-tinted ring via{" "}
        <code style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>
          --color-gw-focus-ring
        </code>{" "}
        (rgba(212, 64, 64, 0.2)). Tab through the elements below to see the ring
        in context. The ring uses{" "}
        <code style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>
          box-shadow
        </code>{" "}
        — not{" "}
