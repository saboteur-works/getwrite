/**
 * @module store/transport/native-entity-cooccurrence-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-entity-mention-counts-backend.web-stub.ts`. Turbopack cannot
 * statically prove that `lib/api/entity-cooccurrence.ts`'s
 * `await import("../../store/transport/native-entity-cooccurrence-backend")`
 ...
 * The real `native-entity-cooccurrence-backend.ts` transitively imports
 ...
 * `resolveEntityCooccurrenceTransport()` only calls into the native branch
 ...
 * `native-entity-cooccurrence-backend.ts` directly (this alias is a
 ...
 */
import type { EntityCooccurrenceTransport } from "../../lib/api/entity-cooccurrence";
 ...
 */
export function createNativeEntityCooccurrenceTransport(): EntityCooccurrenceTransport {
  throw new Error(
    "native-entity-cooccurrence-backend.web-stub: the native entity-" +
      "cooccurrence transport was reached in a web/desktop build. This " +
