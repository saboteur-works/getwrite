...
export async function persistSavedQuery(
    ...
    if (!response.ok) {
        const errorBody = await response.json().catch(() => ({}));
        throw new Error(
            getApiErrorMessage(errorBody, "Failed to save query."),
        );
    }
...
export async function removeSavedQuery(
    ...
    if (!response.ok) {
        const errorBody = await response.json().catch(() => ({}));
        throw new Error(
            getApiErrorMessage(errorBody, "Failed to delete query."),
        );
    }
}
...
export async function evaluateQueryAst(
    ...
    if (!response.ok) {
        const errorBody = await response.json().catch(() => ({}));
        throw new Error(
            getApiErrorMessage(errorBody, "Query evaluation failed."),
        );
    }
