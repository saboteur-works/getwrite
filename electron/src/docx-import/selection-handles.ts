 ...
 *
 * Tracks the one active DOCX import source a user has picked via a native
 * file dialog (either a single `.docx` file or a folder of them), without
 * ever handing the absolute path back to a renderer.
 *
 ...
 * opaque handle string. Later main-process code resolves that handle back to
 * the real path when it actually needs it, and consumes it once an import
 * starts so the same handle cannot be reused for a second import.
 *
 * A new, DOCX-specific module — deliberately not shared with or generalized
 * from `electron/src/scrivener-import/selection-handles.ts`, which this file
...
export interface SelectionHandleTarget {
   ...
   * {@link SelectionHandleRegistry.resolve}. */
  path: string;
  /** Name to show the user in place of the raw path. */
  displayName: string;
}

/** Tracks the single active DOCX import selection handle. */
export interface SelectionHandleRegistry {
   ...
   * one DOCX import selection can be "active" at a time.
   *
   * @param path - Absolute path to the selected `.docx` file or folder.
...
/**
 * Creates a registry tracking one active DOCX import selection at a time.
 *
