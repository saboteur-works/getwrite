/**
 * The main-process registry that tracks the one active DOCX import selection
 * handle. Runtime-free, so this runs against plain in-memory state — no
 * filesystem, no Electron. Mirrors
 * `electron/tests/scrivener-import/selection-handles.test.ts`'s own coverage
 * shape against the DOCX-specific module.
 */
import { describe, it, expect } from "vitest";
import { createSelectionHandleRegistry } from "../../src/docx-import/selection-handles";
...
describe("createSelectionHandleRegistry", () => {
  it("returns an opaque handle that never encodes the path, with the display name retrievable via resolve", () => {
    ...
    const path = "/Users/writer/Manuscripts/My Novel.docx";
  ...
  it("resolves a valid, unconsumed handle to its path and display name", () => {
    const registry = createSelectionHandleRegistry();
    const handle = registry.record("/a/b/Chapters", "Chapters");
  ...
  it("invalidates the prior handle when a second selection is recorded", () => {
    const registry = createSelectionHandleRegistry();
    const first = registry.record("/a/First.docx", "First");
    const second = registry.record("/b/Second.docx", "Second");
  ...
  it("returns false and does not re-fire when consuming an already-consumed handle", () => {
    const registry = createSelectionHandleRegistry();
    const handle = registry.record("/a/Manuscript.docx", "Manuscript");
