 ...
 * fires first on a forked DOCX import worker, and must never settle a second
 * time once one of them has. Mirrors
 * `electron/tests/scrivener-import/await-worker-outcome.test.ts`'s own
 * coverage shape against the DOCX-specific module, using the DOCX pipeline's
 * own `ImportOutcome` shape.
 */
import { describe, it, expect, vi } from "vitest";
import {
  awaitWorkerOutcome,
  type WorkerLike,
} from "../../src/docx-import/await-worker-outcome";
import type { ImportOutcome } from "../../src/docx-import/handle-import-request";
...
describe("awaitWorkerOutcome", () => {
  ...
  it("resolves with a generic, path-free fatal outcome when the worker errors before any message, without leaking the raw error message", async () => {
    const { worker, emit } = createFakeWorker();
    const error = new Error(
      'ENOENT: no such file or directory, open "/private/tmp/foo/Old.docx"',
    );
